...
SchroedingerProblem<dim>::run ()
{
  ...
  const double ke_term = (gubbins::HBAR*gubbins::HBAR) / (2.*gubbins::mstar_p_CdTe*gubbins::M0);
  ...
  for (unsigned int i=0; i<eigenvalues.size (); ++i)
    {
      ...
      const double analytical_value = (gubbins::HBAR*gubbins::HBAR*gubbins::PI*gubbins::PI*n*n) / (2*gubbins::mstar_p_CdTe*gubbins::M0*L*L);
      std::cout << analytical_value/(1e-03*gubbins::E0) << " ";
    }
  std::cout << std::endl;
}
